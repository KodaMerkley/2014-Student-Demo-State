...
data <- merge(hd2014[,c(1:2,5)], EFFY2014)
    ##Subsetting on schools with over 500 students to get rid of salons
    data <- subset(data, data$EFYTOTLT >= 500)
...
shinyServer(function(input, output) {
    ...
    output$distPlot <- renderPlot({
        ...
        g <- ggplot(data = subset(get(Data), (STABBR %in% st)&(EFFYLEV %in% deg)
            ), aes(y = value, x = INSTNM, fill = Group))
            g <- g + theme(axis.text.x = element_text(size = rel(1.5), angle = 90))
            g <- g + xlab("States") + ylab("%")
            g <- g +geom_bar(stat = "identity",  position = pstn, alpha = .75)
